...
3. Policy
      ...
      - AWS Config deployed in account, can reacts to resource config and compliance change and send to cloudwatch, it can schedule run daily
...
4. Roles 
      ...
      - Assume role, for other accounts access your stuff, you can create a role for trusting acct, trusted acct and assume that role and access the trusting acct stuff. Obtain temporary security credentials STS to make API calls. 
...
5. EC2 instance Roles, access other services on your behalf. 
6. Launch Types(On-demand, Spot Instances, Spot Fleet)
...
9. Hibernate
10. EBS storage (Elastic Block Store)
...
13. AWS Batch schedule run parallel full range of AWS compute services and features, such as AWS Fargate, Amazon EC2 and Spot Instances.

### High Availability and Scalability: ELB & ASG

1. Elastic Load Balancing types (select ec2s as target)
...
- Glacier
    - Long-term storage, has vault lock to meet compliance
...
1. Strong read-after-write consistency automatically. As soon as you write, you will be able to read the latest version.
2. S3 sync command to copy objects between s3 buckets. Up to many PBs?
3. S3 console cannot be used to transfer 1PB of data from one bucket to another. 
4. Cross-region replication is for async copying new objects, Minimize latency for different region user access it.  
...
### Redshift
1. Data warehouse for large scale data set analysis and storage. Redshift spectrum can quickly run query against s3 without load data to Redshift. 
2. You can deploy to multi-region or multi-availability zone (AZ) clusters.
3. Snapshots are point-in-time backups of a cluster, automated and manual. Amazon Redshift stores these snapshots internally in Amazon S3. You can set copying snapshots to another AWS Region. 
...
3. RDS for Oracle read replicas when running mission-critical databases with a business requirement for your DR configuration to span across different Regions. At the same time, you want to use your DR investments to handle some of your production read workloads in another Region closer to the users.
4. RDS for Oracle mounted DB replicas are ideal if you just need DR but don’t have the requirement to route your read workloads to the replica.
5. RPO(recover point, the lastest backup) and RTO(recover time, how long service back online) with Amazon RDS for Oracle’s managed HA and DR capabilities
...
3. PrivateLink = VPC endpoints + Gateway endpoints
4. AWS Transit Gateway connects VPCs and on-premises networks through a central hub.
